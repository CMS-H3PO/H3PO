...
```
Some 2018 signal samples required more memory than the default 2000 MB so those were resubmitted with the requested memory raised to 4000 MB and the output directory set to the one from the original submission (note the `--no_timestamp` input argument)
```
