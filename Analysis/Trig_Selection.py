...
def Event_selection(fname,process,event_counts,eventsToRead=None):
    ...
    # "ref" - for reference trigger
    trig = events[events.HLT.AK8PFJet260]
    # ref + trig - for trigger efficirncy
    
    #ref = events[events.HLT.AK8PFJet260]
    #trig = ref[ref.HLT.PFHT1050]
    # print list of triggers - print(events.HLT)
    ...
    events_preselection_eq2 =  trig[ak.num(fatjets, axis=1)==2]
    fatjets_eq2             = fatjets[ak.num(fatjets, axis=1)==2]

    # select events with at least 3 preselected fat jets
    events_preselection =  trig[ak.num(fatjets, axis=1)>2]
