...
r_txt = open('/users/ferencek/HHH/H3PO/Analysis/HHH_samples_2017.txt', 'r')
...
for r in remote_files:
    ...
    dest = f'/STORE/matej/H3_skims/2017/XToYHTo6B_{mx}_{my}'
    
    cmd = f"mkdir -p {dest}"
    print('\n' + cmd)
    os.system(cmd)

    cmd = f'xrdcp -f "root://eoscms.cern.ch/{r}" "{dest}"'
    print(cmd + '\n')
